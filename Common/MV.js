...
//------------------------------------------------------------------------------
//
// Returns the upper 3x3 matrix from the 4x4 matrix 
//
function mat4to3( m4)
{
    var m3 = [
        vec3(m4[0][0], m4[0][1], m4[0][2]),
...
//---------------------------------------------------------------------------
// multiply a  matrix with a vec4
function multv(m, v)
{
    var result = [];
    if(! m.matrix ) {
        throw "mul4v: input must be a matrix and a vector"
    }
...
{
    if( !m.matrix ) {
        throw "inverse(): trying to inverse a non-matrix";
    }

    if(m.length != 4) {
        throw "inverse(): must be 4x4 matrix";
    }

    var a = mult(mat4(), m);
    var b = mat4();         // identity matrix
    ...
    for( c = 0; c < 4; c++) {
        ...
        rowMax = c;
        for(r = c+1; r < 4; r++) {
            if(Math.abs(a[c][r] > Math.abs(a[c][rowMax]))) {
                rowMax = r;
            }
        }

        // If the max value here is 0, we can't invert.  Return identity.
        if (a[c][rowMax] == 0.0) return mat4();
